...
map.count.data <- map.data2 %>%
  group_by(location) %>%
  summarize(count=n())
map.data <- data.frame(unique(map.data[,c("location")]))
colnames(map.data) <- c("location")
map.count.data <- merge(map.data, map.count.data, by = "location", all=TRUE)
map.count.data[is.na(map.count.data)] <- 0
map.count.data[,1] <- iconv(map.count.data[,1], "", "UTF-8", "?")


#na.omit(count)
# count <- merge(count, map.data, by = "location", all=TRUE)
# map.data %>%
#   group_by(location) %>%
#   summarize()
